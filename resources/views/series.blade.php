...
<head>
    <meta charset="utf-8">
    <title>{{ $series->NAME_SERIES }} - NOTflix</title>
            ...
            <div class="row">
                <div class="col-md-4">
                    ...
                    <div class="mt-3">
                        ...
                        <p>⭐ {{ $series->IMDB_RATE ?? 'N/A' }}/10 ({{ $series->IMDB_RATE_COUNT ?? 0 }} votes)</p>
                ...
                <div class="col-md-8">
                    <h1>{{ $series->NAME_SERIES }}</h1>
                    <p class="text-muted">{{ $series->YEAR ?? 'Unknown Year' }} | {{ $series->DURATION_MIN ?? 'N/A' }} min/episode | {{ $series->NUMBER_OF_EPISODES_IN_SEASON ?? 'N/A' }} episodes</p>
                    ...
                    <div class="row mt-4">
                        <div class="col-md-6">
                            ...
                            <p>{{ $series->LANGUAGE ?? 'N/A' }}</p>
                        </div>
                        <div class="col-md-6">
                            <h5 class="info-label">Budget</h5>
                            <p>${{ number_format($series->BUDGET ?? 0) }}</p>
                        </div>
                        <div class="col-md-6">
                            <h5 class="info-label">Revenue</h5>
                            <p>${{ number_format($series->REVENUE ?? 0) }}</p>
                        </div>
                    </div>

                    @if($series->HOME_PAGE_LINK)
                    <div class="mt-3">
                        <a href="{{ $series->HOME_PAGE_LINK }}" target="_blank" class="btn btn-primary">
                    ...
                    <div class="row">
                        @forelse($series->actors as $actor)
                            <div class="col-md-3 mb-3">
                                <a href="{{ route('actor.show', $actor->ID) }}" class="text-white text-decoration-none">
                                    <div class="text-center">
                                        <img src="{{ asset($actor->IMAGE ?? 'assets/img/actor_pic_not_available.jpg') }}"
